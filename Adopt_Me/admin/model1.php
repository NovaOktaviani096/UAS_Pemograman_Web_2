...
function tambahdatakucing($kategori, $foto, $nama_kucing, $umur, $alamat, $keterangan)
{
    $sql = "INSERT INTO `data_kucing` (`kategori`, `foto`, `nama_kucing`, `umur`, `alamat`, `keterangan`) VALUES (:kategori, :foto, :nama_kucing,:umur,:alamat,:keterangan)";
    $stmt = koneksi()->prepare($sql);
    $result = $stmt->execute(array(':kategori'=> $kategori, ':foto'=> $foto, ':nama_kucing'=> $nama_kucing,':umur'=> $umur,':alamat'=> $alamat,':keterangan'=> $keterangan));
...
function updatekucing($id_kucing, $kategori, $foto, $nama_kucing, $umur, $alamat, $keterangan) 
{
    $pdo_statement = koneksi()->prepare(
        "UPDATE data_kucing SET kategori='" . $kategori ."',foto='" . $foto ."', nama_kucing='" . $nama_kucing . "', umur='" . $umur ."', alamat='" . $alamat ."', keterangan='" . $keterangan . "' WHERE id_kucing = ". $id_kucing
    );
    $result = $pdo_statement->execute();
    if (!empty($result)) {
        header('location:../admin/data_kucing.php');
